...
import { lookup, sellStock } from './api-quote';
...
const SellPage: React.FC = () => {
    ...
    const handleSell = async () => {
        ...
        if (!stocksError && !quoteError && userData) {
            const requestedStock = userStocks.find(stock => stock.symbol === symbol);
            if (!requestedStock || requestedStock.shares < numberOfStocks) {
            ...
            if (userId !== undefined) {
                ...
                if (confirmPurchase) {
                    ...
                    const result = await sellStock(buyModel); // Make sure to await the sell Stock function
                    if (result) {
                        window.alert('Stock sold successfully'); 
                        window.location.reload();

                    } else {
                        window.alert('Failed to sell stock');
        ...
        <div className="Main">
            <div className="mb-3 mb-3-custom-width">
                ...
                <div className="d-flex">
                    ...
                    <button className="btn btn-primary btnsp" onClick={handleSell}>Sell</button>
                </div>
